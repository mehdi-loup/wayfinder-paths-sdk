...
- **Hyperliquid perps/spot/outcomes:** use the per-action MCP tools — `hyperliquid_place_market_order` (IOC), `hyperliquid_place_limit_order` (GTC), `hyperliquid_place_trigger_order` (TP/SL), `hyperliquid_cancel_order`, `hyperliquid_update_leverage`, `hyperliquid_deposit`, `hyperliquid_withdraw`. `asset_name` selects perp (`BTC-USDC`) vs spot (`BTC/USDC`) vs HIP-3 (`xyz:SP500`) vs HIP-4 outcomes (`#<encoding>`); the market/limit tools dispatch the outcome path inline (integer contracts, no builder fee). Order tools don't take leverage — call `hyperliquid_update_leverage` first. Before perp orders, call `hyperliquid_get_state(label=..., asset_name=...)` and use `account_collateral.free_usdc` for account-level free collateral, then `trade_context.available_to_trade_margin_{long,short}_usd`, leverage, max order notional, and live position to size the exact ticket. In UnifiedAccount mode, spot clearinghouse state is the source for balances/holds; `activeAssetData` is the source for side-specific order capacity. Do not treat `tokenToAvailableAfterMaintenance` as available to open more risk. Set `reduce_only=true` for close/reduce flows; use `allow_flip=true` only when the user explicitly wants to flip. **Before your first Hyperliquid write in a session, invoke `/using-hyperliquid-adapter`**.
...
- Generated monitor scripts must use `wayfinder_paths.runner.monitor_state` (`read_monitor_state`, `write_monitor_state`) for durable JSON state under `$WAYFINDER_RUNNER_DIR/job_state/$WAYFINDER_KV_NAMESPACE/`. Never store monitor/checkpoint/alert state in `/tmp`.
