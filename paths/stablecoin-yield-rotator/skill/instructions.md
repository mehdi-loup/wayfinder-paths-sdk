...
- `auto-rotate`: unattended `update` for runner scheduling — executes without interactive confirmation and emails a summary on executions and new failures (no-ops are silent; repeated identical halts alert once). Schedule it with `wayfinder runner add-job --type script --script-path paths/stablecoin-yield-rotator/scripts/main.py --arg --action --arg auto-rotate --interval 3600`. Warn the user that runner jobs are live fund-moving automation outside the safety-review hook before setting one up.
